...
# puzzle_input = "bright indigo bags contain 4 shiny turquoise bags, 3 wavy yellow bags.
...
# def count_shiny_gold_containers(bag_rules)
...
#     outside_hash = {}
#     inside_hash = {}
#     edit = rule.chop.gsub(" bags", '').gsub(" bag", '')
#     edit = edit.split(" contain ")
#     outside_bag = edit[0]
#     inside_bags = edit[1].split(", ")
#     inside_bags.each do |bag|
#       if bag != "no other"
#         qty = bag.slice!(0..1).chop
#         description = bag
#         inside_hash[description] = qty
#       end
#     end
#     if inside_bags != ["no other"] # removes bags that contain nothing
#       outside_hash[outside_bag] = inside_hash
#       rules_ary << outside_hash
#       if inside_bags.include?("shiny gold")
#         compatibles << outside_bag
#       end
#     end
#     compatibles.each do |compatible|
#       if inside_bags.include?(compatible)
#         compatibles << outside_bag
#       end
#     end
#   end
#   rules_ary.each do |rules_hash|
#     rules_hash.delete_if {|k, v|
#       !compatibles.include?(k) && !v.any? {|inner_k, inner_v| compatibles.include?(inner_k)}
#     }
#   end
#   rules_ary.delete_if{|rules_hash| rules_hash == {}}
#   p rules_ary.count
# end

# p count_shiny_gold_containers(puzzle_input)
