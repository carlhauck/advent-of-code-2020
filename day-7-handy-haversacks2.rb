...
def count_bags_inside(bag_rules)
  ...
  bag_rules.each do |rule|
    ...
    if inside_bags != ["no other"] # removes bags that contain nothing
      @rules_hash[outside_bag] = inside_hash
    end
  end
  
  @sum = 0
  check_bag_contents("shiny gold")

  @sum
end

def check_bag_contents(bag)
  if @rules_hash[bag]
    @rules_hash[bag].each {|k, v|
      @sum += v.to_i
      v.to_i.times do
        check_bag_contents(k)
      end
    }
  end
end

p count_bags_inside(puzzle_input)