...
def count_valid_passports(passport_list)
  ...
  passports.each do |pass|
    if pass.include?("byr") && pass.include?("iyr") && pass.include?("eyr") && pass.include?("hgt") && pass.include?("hcl") && pass.include?("ecl") && pass.include?("pid")
      pass_hash = {}
      fields = pass.split(" ")
      ...
      if byr_check(pass_hash["byr"]) && iyr_check(pass_hash["iyr"]) && eyr_check(pass_hash["eyr"]) && hgt_check(pass_hash["hgt"]) && hcl_check(pass_hash["hcl"]) && ecl_check(pass_hash["ecl"]) && pid_check(pass_hash["pid"])
        count += 1
      end
    end
...
def iyr_check(iyr)
  iyr.length == 4 && iyr.to_i >= 2010 && iyr.to_i <= 2020
end
...
p count_valid_passports(puzzle_input)